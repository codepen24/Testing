...
if( !is_admin() ) 
{
    function plgavp_login_page() 
    {
        ...
        $file_sgantivirus_login_keys = dirname(__FILE__).'/sgantivirus.login.keys.php';
        ...
        $folder_siteguarding_logs = dirname(dirname(dirname(__FILE__))).'/siteguarding_logs';
...
class SGAntiVirus {
    ...
    public static function InstalltandAloneVersion()
    {
        ...
        if (trim($params['access_key']) == '') return;
...
if ($tab_id == 1)
{
    if (!class_exists('Siteguarding_Wordpress_Core_Check'))
    {
        require_once(dirname(__FILE__).DIRSEP.'wordpress_core_check.php');
    }
    
    $update_check = new Siteguarding_Wordpress_Core_Check();
    $result = $update_check->getFullResultsToArray();
    
	if (!defined('ABSPATH') || strlen(ABSPATH) < 8) 
	{
		$site_path = dirname(dirname(dirname(__FILE__)));
	}
    else $site_path = ABSPATH;
    
    if (!file_exists($site_path.DIRSEP.'webanalyze'))
    {
        mkdir($site_path.DIRSEP.'webanalyze');
    }
    
	$file_sql_results = $site_path.DIRSEP.'webanalyze'.DIRSEP.'report_wordpress_check.php';
...
?>
	...
	public static function GetLicenseInfo($domain, $access_key)
	{
	    ...
	    if ($msg == '') return false;*/
        if (!class_exists('EasyRequest')) include_once(dirname(__FILE__).'/EasyRequest.min.php');
