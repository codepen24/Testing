...
function scan()
{
    ...
    if (isset($_REQUEST['full_scan']) && intval($_REQUEST['full_scan']) == 1) $scanner->full_scan = true;
...
function do_remote_scan()
{
    $remote_do_id = trim($_REQUEST['remote_id']);
    if ($remote_do_id == '') return;
    
    $link = SITEGUARDING_SERVER.'?action=do_remote_scan&remote_id='.$remote_do_id;
    $destination = dirname(__FILE__).DIRSEP.'antivirus_scan.txt';
    $result = CreateRemote_file_contents($link, $destination);
    if ($result === false || $result == 0) die('Cant download the commands');
...
function ZipFilesList($zip_filename, $files_list)
{
    ...
    foreach ($files_list as $file_name_short) 
    {
    	...
    	if (filesize($file_name) > 0) $zip->addFile(fread($handle, filesize($file_name)), $file_name_short, filectime($file_name), NULL, TRUE, ZipSGAVP::getFileExtAttr($file_name));
    	fclose($handle);
    }
    $zip->finalize();
...
class ZipSGAVP {
    ...
    private function buildZipEntry($filePath, $fileComment, $gpFlags, $gzType, $timestamp, $fileCRC32, $gzLength, $dataLength, $extFileAttr) {
		...
		if ($this->addExtraField) {
            $localExtraField .= "\x55\x54\x09\x00\x03" . $tsPack . $tsPack . ZipSGAVP::EXTRA_FIELD_NEW_UNIX_GUID;
			$centralExtraField .= "\x55\x54\x05\x00\x03" . $tsPack . ZipSGAVP::EXTRA_FIELD_NEW_UNIX_GUID;
		}
...
{
	static public function DetectCMS()
	{
	...
	static public function GetSQLsettings($cms = '')
	{
        $a = array(
            'sql_server' => '',
            'sql_database' => '',
	...
	static public function ScanSQLContent($domain, $sql_settings = array(), $cms = '')
	{
        $a = array();
    ...
    static public function SQL_query($con, $sql)
    {
...
class SGAntiVirus_scanner
{
	...
	public function scanner($check_session = true, $show_results = true)
	{
        ...
        if (!defined('FULL_SCAN')) define('FULL_SCAN', $this->full_scan);
		...
		if (file_exists($this->tmp_dir.'report_notreadable_files.php')) unlink($this->tmp_dir.'report_notreadable_files.php');
		...
		if (file_exists($this->tmp_dir.'report_wordpress_check.php')) unlink($this->tmp_dir.'report_wordpress_check.php');
        ...
        if (FULL_SCAN) $error_msg = 'FULL_SCAN: true';
        else $error_msg = 'FULL_SCAN: false';
		if (self::$debug) self::DebugLog($error_msg);
        ...
        {
            $error_msg = 'Found Lock file: antivirus_sql.lock';
            if (self::$debug) self::DebugLog($error_msg);
        ...
        if ($SCAN_SQL) 
        {
            ...
            if ($sql_results['status'] === true)
            {
                ...
                // Check core, plugins, themes, admins
                if ($detected_CMS == 'wordpress')
                {
                    $error_msg = 'SQL WordPress check: core, plugins, themes, admins';
                    if (self::$debug) self::DebugLog($error_msg);
                    
                    $check_wordpress = new Siteguarding_Wordpress_Core_Check($sql_settings);
                    $result = $check_wordpress->getFullResultsToArray();
                    
            		$file_sql_results = $this->tmp_dir.'report_wordpress_check.php';
            		$fp = fopen($file_sql_results, 'w');
            		$status = fwrite($fp, "<?php die(); ?>\n".json_encode($result));
            		fclose($fp);
                    
                    unset($check_wordpress);
                    unset($result);
                }
            ...
            while ($file = readdir($currentDir)) 
            {
                ...
                if (is_dir($file)) 
                {
					...
					if (strpos($file, '.php') || strpos($file, '.phtml'))
					{
                        ...
                        if (!is_readable($file))
                        {
                            self::DebugLog('NOT readable: '.$file);
                            self::NotReadableFilesLog(str_replace($this->scan_path, "", $file));
                            continue;
                        }
                        ...
                        switch ($ext)
                        {
                            ...
                            default:
                                ...
                                {
            					    $file_size = filesize($file);
                                    if ($file_size > self::$max_filesize) continue;
                                    ...
                                    if ($contents == '<?php')
                                    {
                                        ...
                                        self::DebugLog('*PHP code detected: '.str_replace($this->scan_path, "", $file).' ['.$file_size.' bytes]');
                                    }
                                }
...
';
	...
	public function NotReadableFilesLog($txt)
	{
        $file = $this->tmp_dir.'report_notreadable_files.php';
        if (!file_exists($file)) $txt = "<?php die(); ?>\n".$txt;
	    $fp = fopen($file, 'a');
		fwrite($fp, $txt."\n");
		fclose($fp);
...
class Siteguarding_Wordpress_Core_Check {
	...
	public function __construct($sql_settings) 
    {
        $this->database = $sql_settings['sql_database'];
		$this->sql_user = $sql_settings['sql_username'];
		$this->sql_pass = $sql_settings['sql_password'];
		$this->host = $sql_settings['sql_server'];
		$this->table_prefix = $sql_settings['prefix'];
		
		$this->db = @mysqli_connect($this->host, $this->sql_user, $this->sql_pass, $this->database);
	...
	public function checkCoreWP() {
		...
		$wp_version = '';
		$lastVersion = '';
		@require_once(SCAN_PATH . '/wp-includes/version.php');
		$lastVersion = @json_decode(@file_get_contents('https://api.wordpress.org/core/version-check/1.7/'))->offers[0]->version;

		$this->core['current_ver'] = $wp_version ? $wp_version : 'unknown';
		$this->core['latest_ver'] = $lastVersion ? $lastVersion : 'unknown';
	...
	public function checkPlugins() {
		...
		$activeSlugs = array();
		$sql = "SELECT option_value FROM " . $this->table_prefix . "options WHERE option_name = 'active_plugins'";
		$result = mysqli_query($this->db, $sql);
		$row = mysqli_fetch_row($result);
		$activePlugins = unserialize($row[0]);
		foreach ($activePlugins as $plugin) {
			$tmpslug = explode("/", $plugin);
			$activeSlugs[] = $tmpslug[0];
		}
		
		$sql = "SELECT option_value FROM " . $this->table_prefix . "options WHERE option_name = '_site_transient_update_plugins'";
		$result = mysqli_query($this->db, $sql);
		$row = mysqli_fetch_row($result);
		$update_plugins = unserialize($row[0]);
		...
		if ($update_plugins && !empty($update_plugins->response)) {
			foreach ($update_plugins->response as $plugin => $vals) {
				...
				$pluginFile = SCAN_PATH . 'wp-content' . DIRSEP . 'plugins' . DIRSEP . $plugin;
				...
				$pluginData = $this->get_file_data($pluginFile, array('Name' => 'Plugin Name', 'Version' => 'Version'));
	...
	public function checkAdmins() {
        $sql =  "SELECT a.user_login, a.user_email FROM {$this->table_prefix}users a INNER JOIN {$this->table_prefix}usermeta b ON ( a.ID = b.user_id ) WHERE b.meta_value LIKE '%\"Administrator\"%'";

		$result = mysqli_query($this->db, $sql);
		while ($row = mysqli_fetch_assoc($result)) {
			$this->admins[] = array('username' => $row['user_login'], 'email' => $row['user_email']);
		}
		return $this;
	}
	...
	public function checkThemes() {
		...
		$sql = "SELECT option_value FROM " . $this->table_prefix . "options WHERE option_name = '_site_transient_update_themes'";
		$result = mysqli_query($this->db, $sql);
		$row = mysqli_fetch_row($result);
		$update_themes = unserialize($row[0]);		
		
		
		
		$sql = "SELECT option_value FROM " . $this->table_prefix . "options WHERE option_name = 'template'";
		$result = mysqli_query($this->db, $sql);
		$row = mysqli_fetch_row($result);
		$current = $row[0];
