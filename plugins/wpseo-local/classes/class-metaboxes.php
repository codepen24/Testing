...
if ( ! class_exists( 'WPSEO_Local_Metaboxes' ) ) {
	...
	class WPSEO_Local_Metaboxes {
		...
		function metabox_locations() {
			...
			echo '<p class="yoast-local-seo-field-desc">' . sprintf( __( 'If your business type is not listed, please read %1$sthe FAQ entry%2$s.', 'yoast-local-seo' ), '<a href="http://kb.yoast.com/article/49-my-business-is-not-listed-can-you-add-it" target="_blank">', '</a>' ) . '</p>';
		...
		function enqueue_scripts() {
			...
			if ( 'wpseo_locations' == get_post_type() ) {
				wp_enqueue_script( 'wp-seo-local-seo-locations', plugins_url( '../js/wp-seo-local-analysis-locations-530' . WPSEO_CSSJS_SUFFIX . '.js', __FILE__ ), array(), WPSEO_VERSION, true );
				...
				wp_enqueue_script( 'wp-seo-local-seo-pages', plugins_url( '../js/wp-seo-local-analysis-pages-450' . WPSEO_CSSJS_SUFFIX . '.js', __FILE__ ), array(), WPSEO_VERSION, true );
		...
		function localize_script_locations() {
			...
			return array(
				...
				'address_schema'      => __( 'Your content contains an address shortcode which contains Schema.org markup. Well done!', 'yoast-local-seo' ),
				'no_address_schema'   => __( 'Your content does not contain an address shortcode which contains Schema.org markup, you should really add that.', 'yoast-local-seo' ),
			);
		}
