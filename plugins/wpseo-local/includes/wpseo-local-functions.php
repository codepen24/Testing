...
function wpseo_local_show_address( $atts ) {
	...
	*/
	...
	foreach ( $locations as $location ) {
		...
		$output = '<div id="wpseo_location-' . esc_attr( $atts['id'] ) . '" class="wpseo-location" itemscope itemtype="http://schema.org/' . ( ( $is_postal_address ) ? 'PostalAddress' : esc_attr( $location['business_type'] ) ) . '">';
		// Show URL as hidden meta, when URL is not visible, since it's required by Schema.org markup.
		if ( false === $atts['show_url'] ) {
			$output .= '<meta itemprop="url" content="' . esc_url( $location['business_url'] ) . '">';
		}
		// Add the business location image if available.
		if ( isset( $location['business_image'] ) ) {
			$output .= '<meta itemprop="image" content="' . wp_get_attachment_image_url( $location['business_image'] ) . '">';
		}
		if ( false == $atts['hide_name'] ) {
			$pt_object = get_post_type_object( 'wpseo_locations' );
			$output .= $tag_title_open . ( ( $atts['from_sl'] && $pt_object->public ) ? '<a href="' . esc_url( $location['business_url'] ) . '">' : '' ) . '<span class="wpseo-business-name" itemprop="name">' . esc_html( $location['business_name'] ) . '</span>' . ( ( $atts['from_sl'] ) ? '</a>' : '' ) . $tag_title_close;
		}
		else {
			// Add a meta with the business name if it shouldn't be displayed to make sure schema.org still validates.
			$output .= '<meta itemprop="name" content="' . esc_html( $location['business_name'] ) . '">';
		}
		...
		if ( $atts['show_logo'] && ! empty( $location['business_logo'] ) && is_numeric( $location['business_logo'] ) ) {
			...
			$output .= wp_get_attachment_image( $location['business_logo'], apply_filters( 'yoast_seo_local_business_logo_size', 'full' ), null, array( 'itemprop' => 'url' ) );
			$output .= '</figure>';
		}
		...
		if ( $atts['show_country'] && ! empty( $location['business_country'] ) ) {
			$output .= '<' . ( ( $atts['oneline'] ) ? 'span' : 'div' ) . '  class="country-name" itemprop="addressCountry">' . WPSEO_Local_Frontend::get_country( $location['business_country'] ) . '</' . ( ( $atts['oneline'] ) ? 'span' : 'div' ) . '>';
		}
		...
		foreach ( $business_contact_details as $order => $details ) {
			if ( 'phone' == $details['key'] && $atts['show_phone'] && ! empty( $location['business_phone'] ) ) {
				...
				$details_output .= sprintf( '<span class="wpseo-phone">%s: <a href="' . esc_url( 'tel:' . preg_replace( '/[^0-9+]/', '', $location['business_phone'] ) ) . '" class="tel"><span itemprop="telephone">' . esc_html( $location['business_phone'] ) . '</span></a></span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
			...
			if ( 'fax' == $details['key'] && $atts['show_fax'] && ! empty( $location['business_fax'] ) ) {
				...
				$details_output .= sprintf( '<span class="wpseo-fax">%s: <span class="tel" itemprop="faxNumber">' . esc_html( $location['business_fax'] ) . '</span></span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
			if ( 'email' == $details['key'] && $atts['show_email'] && ! empty( $location['business_email'] ) ) {
				/* translators: %s extends to the label for e-mail */
				$details_output .= sprintf( '<span class="wpseo-email">%s: <a href="' . esc_url( 'mailto:' . antispambot( $location['business_email'] ) ) . '" itemprop="email">' . antispambot( esc_html( $location['business_email'] ) ) . '</a></span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
			if ( 'url' == $details['key'] && $atts['show_url'] ) {
				/* translators: %s extends to the label for business url */
				$details_output .= sprintf( '<span class="wpseo-url">%s: <a href="' . esc_url( $location['business_url'] ) . '" itemprop="url">' . esc_html( $location['business_url'] ) . '</a></span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
			if ( 'vat' == $details['key'] && $atts['show_vat'] && ! empty( $location['business_vat'] ) ) {
				/* translators: %s extends to the label for businss VAT number */
				$details_output .= sprintf( '<span class="wpseo-vat">%s: <span itemprop="vatID">' . esc_html( $location['business_vat'] ) . '</span></span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
			if ( 'tax' == $details['key'] && $atts['show_tax'] && ! empty( $location['business_tax'] ) ) {
				/* translators: %s extends to the label for business tax number */
				$details_output .= sprintf( '<span class="wpseo-tax">%s: <span itemprop="taxID">' . esc_html( $location['business_tax'] ) . '</span></span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
			if ( 'coc' == $details['key'] && $atts['show_coc'] && ! empty( $location['business_coc'] ) ) {
				/* translators: %s extends to the label for business COC number*/
				$details_output .= sprintf( '<span class="wpseo-vat">%s: ' . esc_html( $location['business_coc'] ) . '</span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
			if ( 'price_range' == $details['key'] && $atts['show_price_range'] && ! empty( $location['business_price_range'] ) ) {
				...
				$details_output .= sprintf( '<span class="wpseo-price-range">%s: <span itemprop="priceRange">' . esc_html( $location['business_price_range'] ) . '</span></span>' . ( ( $atts['oneline'] ) ? ' ' : '<br/>' ), esc_html( $details['label'] ) );
			}
		}
...
function wpseo_local_show_opening_hours( $atts, $show_schema = true, $standalone = true ) {
	...
	foreach ( $locations as $location ) {
		...
		if ( true == $standalone ) {
			$output .= '<div class="wpseo-opening-hours-wrapper" itemscope itemtype="http://schema.org/' . esc_attr( $location['business_type'] ) . '">';
			$output .= '<meta itemprop="name" content="' . esc_attr( $location['business_name'] ) . '">';
			$output .= '<meta itemprop="address" content="' . esc_attr( $location['business_address'] ) . '">';
			$output .= '<meta itemprop="telephone" content="' . esc_attr( $location['business_phone'] ) . '">';
			$output .= '<meta itemprop="pricerange" content="' . esc_attr( $location['business_price_range'] ) . '">';
			// Add the business location image if available.
			if ( isset( $location['business_image'] ) ) {
				$output .= '<meta itemprop="image" content="' . wp_get_attachment_image_url( $location['business_image'] ) . '">';
			}
		...
		foreach ( $days as $key => $day ) {
			...
			if ( wpseo_has_multiple_locations() ) {
				...
				$value_from        = get_post_meta( $location['post_id'], $field_name . '_from', true );
				$value_to          = get_post_meta( $location['post_id'], $field_name . '_to', true );
				$value_second_from = get_post_meta( $location['post_id'], $field_name . '_second_from', true );
				$value_second_to   = get_post_meta( $location['post_id'], $field_name . '_second_to', true );
			...
			$output .= '<tr itemprop="openingHoursSpecification" itemscope="" itemtype="http://schema.org/OpeningHoursSpecification">';
			$output .= '<td class="day" itemprop="dayOfWeek" href="http://schema.org/' . ucfirst( $key ) . '">' . $day . '</td>';
			$output .= '<td class="time">';

			$output_time = '';
			if ( $value_from != 'closed' && $value_to != 'closed' ) {
				$output_time .= '<span content="' . $day_abbr . ' ' . $value_from . '-' . $value_to . '"><time itemprop="opens" content="' . $value_from . '">' . $value_from_formatted . '</time> - <time itemprop="closes" content="' . $value_to . '">' . $value_to_formatted . '</time></span>';
			}
			else {
			...
			if ( $multiple_opening_hours ) {
				if ( $value_from != 'closed' && $value_to != 'closed' && $value_second_from != 'closed' && $value_second_to != 'closed' ) {
					...
					$output_time .= '<span content="' . $day_abbr . ' <time itemprop="opens" content="' . $value_second_from . '">' . $value_second_from_formatted . '</time> - <time itemprop="closes" content="' . $value_second_to . '">' . $value_second_to_formatted . '</time></span>';
				}
				else {
					$output_time .= '';
				}
...
function wpseo_enqueue_geocoder() {
	...
	if ( $wpseo_enqueue_geocoder ) {
		...
		$script_name = 'wp-seo-local-frontend-600.min.js';
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			$script_name = 'wp-seo-local-frontend-600.js';
		}
