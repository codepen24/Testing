...
function ia_woocommerce_options() {
	...
	<div class="options_group ia-woosubs" style="margin-bottom: 20px;">
		<?php
			...
			if(version_compare( WOOCOMMERCE_VERSION, '3.0', '>=' )) {
		 		...
		 		$pre_opt = array();
		 		if($wid > 0) $pre_opt[$wid] = 'Action Set # ' . $wid;
	...
	<?php if(version_compare( WOOCOMMERCE_VERSION, '3.0', '>=' )) { ?>
		<script type="text/javascript" src="<?php echo INFUSEDWOO_PRO_URL . "admin-menu/assets/paneledits3.js?ver=3.8.7"; ?>"></script>
	<?php } else { ?>
...
function ia_woocommerce_process_product( $post_id, $post = null  ) {
	...
	if ( $post->post_type == "product" ) {
		...
		if(isset($_POST['iw_product_panel'])) {
			...
			update_post_meta( $post_id, 'iw_override_price_html', $_POST['iw_override_price_html']);
			update_post_meta( $post_id, 'infusionsoft_sub_activated', 	$_POST['infusionsoft_sub_activated']);
			update_post_meta( $post_id, 'infusionsoft_sub_cancelled', 	$_POST['infusionsoft_sub_cancelled']);
			update_post_meta( $post_id, 'infusionsoft_sub_on-hold', 	$_POST['infusionsoft_sub_on-hold']);
			update_post_meta( $post_id, 'infusionsoft_sub_expired', 	$_POST['infusionsoft_sub_expired']);
		}
	}
}
...
function iw_infusion_actions() {
	...
	$items = array();
	if(is_array($fetch)) foreach($fetch as $f) {
