<?php
	...
	function iw_typagecontrol() {
		...
		if(is_order_received_page()) {
			...
			$passvars = array(
					'FirstName'			=> stripslashes($order->get_billing_first_name()),
					'LastName'			=> stripslashes($order->get_billing_last_name()),
					'Email'				=> stripslashes($order->get_billing_email()),
					'StreetAddress1' 	=> stripslashes($order->get_billing_address_1()),
					'StreetAddress2' 	=> stripslashes($order->get_billing_address_2()),
					'City' 				=> stripslashes($order->get_billing_city()),
					'State' 			=> stripslashes($order->get_billing_state()),
					'Country' 			=> stripslashes(iw_to_country($order->get_billing_country())),
					'PostalCode' 		=> stripslashes($order->get_billing_postcode()),
					'Address2Street1' 	=> stripslashes($order->get_shipping_address_1()),
					'Address2Street2' 	=> stripslashes($order->get_shipping_address_2()),
					'City2' 			=> stripslashes($order->get_shipping_city()),
					'State2' 			=> stripslashes($order->get_shipping_state()),
					'Country2' 			=> stripslashes(iw_to_country($order->get_shipping_country())),
					'WooOrderId'		=> $orderid,
				);
		...
		foreach($conds as $c) {
			...
			if($type == 'always') {
				...
				foreach($items as $item) {
					...
					if(in_array($item['variation_id'], $checks)) {
						$sub_check = true;
					}
					if($sub_check) break;
				}
				...
				$sub_check = ($order->get_payment_method() == $checks);
			}
