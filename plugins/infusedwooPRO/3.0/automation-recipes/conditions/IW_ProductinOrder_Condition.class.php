...
class IW_ProductinOrder_Condition extends IW_Automation_Condition {
	function get_title() {
		return 'When order contains certain products...';
	}
	...
	function on_class_load() {
		add_action( 'adm_automation_recipe_after', array($this, 'js_script'));
	}
	...
	function js_script() {
		...
		<script> 
				jQuery("body").on('change','.wptype-sel', function() {
					...
					} else if(jQuery(this).val() == "wptype") {
...
iw_add_condition_class('IW_ProductinOrder_Condition');