...
class IW_WooSubPreviousStatus_Condition extends IW_Automation_Condition {
	...
	function test($config, $trigger) {
		if($trigger->sub_event == 'status_changed') {
			$status1 = str_replace('wc-', '', $config['status1']);
			$status2 = str_replace('wc-', '', $config['status2']);

			$current1 = str_replace('wc-', '', $trigger->pass_vars[2]);
			$current2 = str_replace('wc-', '', $trigger->pass_vars[1]);


			return ((empty($status1) || $status1 == $current1) && (empty($status2) || $status2 == $current2));
		} else {
			return false;
		}
	}
