	...
	public function get_available_conditions($trigger_class = "") {
		...
		foreach($condition_classes as $cond) {
			...
			if(method_exists($cond_obj, 'allowed_triggers')) {
				...
				$trigger_instance = new $trigger_class;

				if(isset($trigger_instance->install_conditions)) {
					if(in_array($cond, $trigger_instance->install_conditions)) $allowed_triggers[] = $trigger_class;
				}
