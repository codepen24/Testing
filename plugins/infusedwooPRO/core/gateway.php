...
function ia_woo_gateway() {
	...
	class IA_WooPaymentGateway extends IW_Payment_Gateway {
		...
		function process_payment($order_id, $cardId = 0) {
			...
			$merchant 	= apply_filters( 'iw_pg_merchant_id', (int) $this->merchant, $order );
			$inv_id     = (int) $inv_info['inv_id'];

			$cardId		= empty($cardId) ? $iw_cache['ia_cardId'] : $cardId;
			if(!empty($cardId)) $cardId = $_SESSION['ia_cardId'];
			...
			if($this->test == "yes") {			
				...
				$orderDate = date('Ymd\TH:i:s', current_time('timestamp'));			
				$iwpro->app->manualPmt($inv_id, $totals, $orderDate, 'SKIPPED - Zero Sale', "Woocommerce Checkout",false);
				$results['Code'] = "APPROVED";
			} else if($this->test != "yes") {
			...
			if(!is_array($results) && $hasproduct) {
				...
				if(!empty($subs)) {
					...
					update_post_meta( $order->get_id(), 'ia_subscriptions', $subs);
				...
				if(version_compare( WOOCOMMERCE_VERSION, '2.1.0', '>=' )) {
					...
					$tyredir = add_query_arg('key', $order->get_order_key(), add_query_arg('order', $order_id, get_permalink($typageid))); 
				}

				do_action( 'infusedwoo_payment_complete_via_infusionsoft', $order->get_id(), $jobid, $inv_id, $subIds, $contactId, $iwpro->app);
		...
		function app_generate_invoice($order) {
			...
			if($iwpro->ia_app_connect()) {
				...
				if(empty($order_num)) $order_num = $order->get_id();
		...
		function app_generate_card($card) {
			...
			if($iwpro->ia_app_connect()) {			
				...
				if(empty($ccidcheck) || $ccidcheck == 0) {
					...
					$iw_cache['ia_cardId'] = (int) $ccidcheck;	
				}
			}		 
		}
		...
		function ia_get_post($name, $else = "") {
			if(isset($_POST[$name])) {
				return !empty($_POST[$name]) ? $_POST[$name] : $else;
			}
			return $else;
		}
