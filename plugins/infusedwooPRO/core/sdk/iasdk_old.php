...
class iaSDKException extends Exception
{
}
...
class iaSDK
{
     ...
     * @return bool
     * @throws iaSDKException
     */

    public function configCon($name, $key = "", $dbOn = "on", $type = "i") {
        $this->cfgCon($name, $key, $dbOn);
    }
    ...
    public function cfgCon($name, $key = "", $dbOn = "on")
    {
        ...
        try {
            ...
            if (strpos($connected, 'ERROR') !== FALSE) {
                throw new iaSDKException($connected);
            }

        } catch (iaSDKException $e) {
            throw new iaSDKException($e->getMessage());
        }
    ...
    public function vendorCon($name, $user, $pass, $key = "", $dbOn = "on")
    {
        ...
        if ($key != "" && $key != "on" && $key != "off" && $key != 'kill' && $key != 'throw') {
            ...
            if (!empty($details[$name])) {
                if ($details[$name][2] == "i") {
                    ...
                    throw new iaSDKException("Invalid application name: \"" . $name . "\"");
                }
            } else {
                throw new iaSDKException("Application Does Not Exist: \"" . $name . "\"");
            }
            $this->key = $details[$name][3];
        }
        ...
        try {
            ...
            throw new iaSDKException("Connection Failed");
        }
        return TRUE;
    }
    ...
    public function methodCaller($service, $callArray)
    {
        ...
        if (!$result->faultCode()) {
            ...
            if ($this->debug == "kill") {
                ...
                throw new iaSDKException($result->faultString(), $result->faultCode());
