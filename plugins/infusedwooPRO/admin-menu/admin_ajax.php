...
function ia_admin_apicreds() {
	...
	if(class_exists('iaSDK')) {
		try {
			...
			$testapp->configCon(trim($_GET['app']), trim($_GET['api']));
			...
			if ($pos === false)  {
				...
				$settings['machinename'] = trim($_GET['app']);
...
function iw_data_search_wooproduct() {
	...
	while ( $query1->have_posts() ) {
		...
		$data[] = array(
				'label' => get_the_title($query1->post->ID) . " [" . $query1->post->ID . "]",
				'value' => $query1->post->ID,
				'id'	=> $query1->post->ID
			);

		$product = get_product($query1->post->ID);

		if(get_class($product) == 'WC_Product_Variable') {
			$children = $product->get_available_variations();

			foreach($children as $child) {
				$var = get_product($child['variation_id']);
				$variation_info = get_the_title($query1->post->ID) . " (";
				...
				$data[] = array(
					'label' => $variation_info . " [" . $child['variation_id'] . "]",
					'value' => $child['variation_id'],
...
function iw_bgp_import_ord($pr=false, $order) {
	global $iwpro;
	...
	foreach($items as $item) {
		if($item['ItemType'] == 1 || $item['ItemType'] == 14) {
			...
			// ADDING TAX DISABLED FROM 3.9.5
...
function iw_bgp_export_ord($pr=false, $oid) {
	global $iwpro;
	$process = $pr;
	$order = get_post($oid);
