				...
				if ( ( ( ! isset( $attributes['course_id'] ) ) || ( empty( $attributes['course_id'] ) ) ) ) {
					if ( learndash_get_post_type_slug( 'course' ) === get_post_type() ) {
						...
						return $this->render_block_wrap( '<span class="learndash-block-error-message">' . sprintf(
							...
							_x( '%1$s ID is required when not used within a %2$s.', 'placeholder: Course, QCourse', 'learndash' ), LearnDash_Custom_Label::get_label( 'course' ), LearnDash_Custom_Label::get_label( 'course' )
						) . '</span>' );
					}
				}
