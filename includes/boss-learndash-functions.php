...
function boss_edu_payment_buttons( $course ) {
	...
	if ( ! empty( $course_price_type ) && $course_price_type == 'closed' ) {
		...
		include_once( LEARNDASH_LMS_PLUGIN_DIR . 'includes/lib/paypal/enhanced-paypal-shortcodes.php' );
...
function boss_edu_course_participants( $args ) {
	 ...
	 * @param int   $course_id
	 */
	$learners = apply_filters( 'boss_learndash_course_participants', $learners, $course_id );
