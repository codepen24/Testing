...
<?php if ( $show_content ) {
    ...
    if ( ( isset( $materials ) ) && ( !empty( $materials ) ) ) :
        ...
        <h4><?php printf( _x( '%s Materials', 'Lesson Materials Label', 'boss-learndash' ), LearnDash_Custom_Label::get_label( 'lesson' ) ); ?></h4>
	...
	if ( !empty( $topics ) ) {
		...
		<div id="learndash_lesson_topics_list">
			<div id="learndash_topic_dots-<?php echo $post->ID; ?>" class="learndash_topic_dots type-list">
				<h2><?php printf( esc_html_x( '%1$s %2$s', 'Lesson Topics Label', 'learndash'), LearnDash_Custom_Label::get_label( 'lesson' ), LearnDash_Custom_Label::get_label( 'topics' ) ); ?></h2>
				<ul>
					<?php
					$odd_class = '';
