...
function wdl_all_members_nav() {
	...
	$tabs = array(
		'0' => array(
			'id'    => 0,
            'title' => __( '会员一览', 'boss_child_theme' ),
		),
	);
			...
			if ( ! empty( $body ) ) {
				...
				if ( ! empty( $_GET['mp_order_id'] ) && ( $mp_order_id = filter_var( $_GET['mp_order_id'], FILTER_SANITIZE_NUMBER_INT ) ) ) {
					...
					if ( $valid_order ) {
						...
						$result  .= '<section class="woocommerce-order-details"><h2 class="woocommerce-order-details__title">' . __( '订单详情', 'boss' ) . '</h2><table class="woocommerce-table woocommerce-table--order-details shop_table order_details"><thead><tr><th class="woocommerce-table__product-name product-name">' . __( '产品', 'boss' ) . '</th><th class="woocommerce-table__product-table product-total">' . __( '金额', 'boss' ) . '</th></tr></thead><tbody>';
						$subtotal = 0;
						...
						<th scope="row">' . __( '小计', 'boss' ) . ':</th><td><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">' . $order_body->currency . '</span> ' . number_format( $subtotal, 2 ) . '</span></td></tr><tr><th scope="row">' . __( '优惠', 'boss' ) . ':</th>
						<td>-<span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">' . $order_body->currency . '</span> ' . $order_body->discount_total . '</span></td></tr><tr><th scope="row">' . __( '总计', 'boss' ) . ':</th><td><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">' . $order_body->currency . '</span> ' . $order_body->total . '</span></td></tr></tfoot></table></section>';
					} else {
					...
					foreach ( $body as $key => $order ) {
						...
						$order_details[ $i ]['status'] = ( __( ucfirst( $order->status ), 'boss_child_theme' ) === 'Processing') ? '进程中' : __( ucfirst( $order->status ), 'boss_child_theme' ) ;
						$i++;
					}
					$result .= '<table style="width: 100%;" id="mp_order_table">';
					$result .= '<thead><tr>';
					$result .= '<th style="padding: 1em;">' . __( '订单号码', 'boss' ) . '</th>';
					$result .= '<th style="padding: 1em;">' . __( '日期', 'boss' ) . '</th>';
					$result .= '<th style="padding: 1em;">' . __( '金额', 'boss' ) . '</th>';
					$result .= '<th style="padding: 1em;">' . __( '状态', 'boss' ) . '</th>';
					$result .= '<th >' . __( '操作', 'boss' ) . '</th>';
					...
					foreach ( $order_details as $key => $value ) {
						...
						$result .= '<td><a href="' . $value['link'] . '" >' . __( '查看订单', 'boss' ) . '</a></td></tr>';
					}
					$result .= '</tbody></table>';
					$result .= '<script>jQuery(\'#mp_order_table\').dataTable( { "scrollX": true, "lengthChange": false, "searching": false, "oLanguage": { "sInfo": "' . __( '列出', 'boss' ) . ' _START_ ' . __( '至', 'boss' ) . ' _END_ ' . __( '总', 'boss' ) . ' _TOTAL_ ' . __( '共', 'boss' ) . '", "oPaginate": { "sFirst": "' . __( 'First', 'boss' ) . '", "sPrevious": "' . __( '前一页', 'boss' ) . '", "sNext": "' . __( '下一页', 'boss' ) . '",  "sLast": "' . __( 'Last', 'boss' ) . '" } } } );</script>';
				}
			}
		} else {
...
if ( file_exists( get_stylesheet_directory() . '/languages' . '/learndash-' . BPLANG . '.mo' ) ) {
    load_textdomain( 'learndash', get_stylesheet_directory() . '/languages' . '/learndash-' . BPLANG . '.mo' );
}


if ( file_exists( get_stylesheet_directory() . '/languages' . '/bbpress-' . BPLANG . '.mo' ) ) {
    load_textdomain( 'bbpress', get_stylesheet_directory() . '/languages' . '/bbpress-' . BPLANG . '.mo' );
}


if ( file_exists( get_stylesheet_directory() . '/languages' . '/buddyboss-wall-' . BPLANG . '.mo' ) ) {
    load_textdomain( 'buddyboss-wall', get_stylesheet_directory() . '/languages' . '/buddyboss-wall-' . BPLANG . '.mo' );
}


if ( file_exists( get_stylesheet_directory() . '/languages' . '/boss-learndash-' . BPLANG . '.mo' ) ) {
    load_textdomain( 'boss-learndash', get_stylesheet_directory() . '/languages' . '/boss-learndash-' . BPLANG . '.mo' );
}


if ( file_exists( get_stylesheet_directory() . '/languages' . '/learndash-propanel-' . BPLANG . '.mo' ) ) {
    load_textdomain( 'learndash-propanel', get_stylesheet_directory() . '/languages' . '/learndash-propanel-' . BPLANG . '.mo' );
}



function remove_login_translation() {
    if( in_array( $GLOBALS['pagenow'], array( 'edit.php', 'admin.php' ) ) ) {
        unload_textdomain('default');
        unload_textdomain('buddypress');
        unload_textdomain('bbpress');
        unload_textdomain('learndash');
        unload_textdomain('buddyboss-wall');
        unload_textdomain('boss-learndash');
    }
}
...
//  global $pagenow;
//  echo'<pre style="position: absolute; z-index: 9999; width: auto; height: auto; background: #000; color: #fff; ">'.print_r($pagenow, 1).'</pre>';
